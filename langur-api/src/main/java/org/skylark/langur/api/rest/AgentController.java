...
import org.skylark.langur.api.dto.CreateAgentRequest;
import org.skylark.langur.api.dto.PlanResponse;
import org.skylark.langur.api.dto.RunAgentRequest;
...
public class AgentController {
    ...
    public ResponseEntity<AgentResponse> createAgent(@RequestBody CreateAgentRequest request) {
        CreateAgentCommand command = CreateAgentCommand.builder()
                ...
                .build();
        return ResponseEntity.ok(apiAssembler.toResponse(agentApplicationService.createAgent(command)));
    }
    ...
    public ResponseEntity<AgentResponse> runAgent(
        ...
        return ResponseEntity.ok(apiAssembler.toResponse(agentApplicationService.runAgent(command)));
    }
    ...
    public ResponseEntity<AgentResponse> getAgent(@PathVariable String agentId) {
        return ResponseEntity.ok(apiAssembler.toResponse(agentApplicationService.getAgent(agentId)));
    }

    @GetMapping
    public ResponseEntity<List<AgentResponse>> listAgents() {
        return ResponseEntity.ok(agentApplicationService.listAgents().stream()
                .map(apiAssembler::toResponse)
                .toList());
    }

    @GetMapping("/runs/{taskId}")
    public ResponseEntity<RunTaskResponse> getRunTask(@PathVariable String taskId) {
        return ResponseEntity.ok(apiAssembler.toResponse(runTaskApplicationService.getTask(taskId)));
    }

    @GetMapping("/{agentId}/runs")
    public ResponseEntity<List<RunTaskResponse>> listRunTasks(@PathVariable String agentId) {
        return ResponseEntity.ok(runTaskApplicationService.listTasksByAgent(agentId).stream()
                .map(apiAssembler::toResponse)
                .toList());
    }
    ...
    private RunAgentCommand buildRunCommand(String agentId, RunAgentRequest request) {
